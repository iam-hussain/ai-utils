...
import { getPromptSetToLoad, type MessageRole } from '@/lib/saved-prompt-sets'
import { getPromptFromLibrary } from '@/lib/prompt-library-load'
...
export default function PromptTestingPage({
  ...
  useEffect(() => {
    const fromLibrary = getPromptFromLibrary()
    if (fromLibrary) {
      setMessages([
        {
          id: `msg-${Date.now()}-${Math.random().toString(36).slice(2)}`,
          type: 'human',
          content: fromLibrary.promptText,
        },
      ])
      return
    }
  ...
  const handleTest = useCallback(() => {
    ...
    if (selectedSkills.length > 0) {
      const skillContent = selectedSkills.map((s: Skill) => `## ${s.name}\n\n${s.content}`).join('\n\n---\n\n')
    ...
    setTestError(null)
    setIsTesting(true)
    socket.emit('test_prompt', { messages: payload, llmProvider })
  ...
  const handleReset = useCallback(async () => {
    const ok = await confirm({
      title: 'Reset',
      description: 'Reset all messages and test result? This cannot be undone.',
      confirmLabel: 'Reset',
      variant: 'destructive',
    })
    if (!ok) return
    setMessages([createBlock('human')])
    setTestResult(null)
    setTestError(null)
  }, [])
  ...
  return (
    ...
    >
      <header className="h-12 shrink-0 border-b flex items-center justify-between gap-2 sm:gap-4 px-4 sm:px-6 bg-background/50 backdrop-blur sticky top-0 z-10">
        <h1 className="font-semibold text-sm shrink-0 truncate min-w-0">Prompt Testing</h1>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-0 min-h-0 overflow-hidden">
        ...
        <div className="border-r flex flex-col min-h-0 overflow-hidden">
          <div className="p-4 sm:p-6 flex flex-col flex-1 min-h-0 gap-4">
              ...
              <CardContent className="flex flex-col flex-1 min-h-0 gap-4 pb-6">
                ...
                <div className="flex flex-col gap-3 shrink-0 pt-2 px-4 py-3 rounded-lg bg-muted/40 border border-border/60">
                  ...
                  <SaveSetDialog
                    ...
                    messages={messages.map((m) => ({ type: m.type, content: m.content, name: m.name, role: m.role }))}
                    onSaved={handleSaved}
                  />
                </div>
              </CardContent>
